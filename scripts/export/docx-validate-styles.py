...
# Pandoc-default styles that SHOULD exist with the right font/size (info only)
# Per-profile style expectations are now in styles/<profile>/docx-validate-styles.py
STYLE_PROFILES = {}
PANDOC_CORE_STYLES = {
...
def validate_styles(docx_path, profile_name="default", project_root=None):
    ...
    if not STYLE_PROFILES:
        print(f"Shared validator has no embedded profiles.")
        print(f"Use styles/{profile_name}/docx-validate-styles.py for profile-specific validation.")
        print(f"Falling back to generic check (Pandoc styles only)...")
        print()

    profile = STYLE_PROFILES.get(profile_name)
    if not profile:
        print(f"No style expectations found for profile '{profile_name}'.")
        print("Skipping per-profile validation.")
        doc = Document(docx_path)
        print(f"Styles in template: {len(doc.styles)}")
        return True
    ...
    for name, expected in profile.get("paragraph", {}).items():
    ...
    for name, expected in profile.get("character", {}).items():
        if name not in style_names:
            print(f"  MISSING  {name}")
            all_ok = False
            continue
